...
echo "Changement hostname definition du nom $NOM_HOST_MACHINE_GLASSFISH1:"
sudo sysctl kernel.hostname=$NOM_HOST_MACHINE_GLASSFISH1
...
sudo echo "#127.0.1.1 $NOM_HOST_MACHINE_GLASSFISH1" >> /etc/hosts
sudo echo "#127.0.0.1 $NOM_HOST_MACHINE_GLASSFISH1" >> /etc/hosts
...
sudo sed -i -e"s@HOSTNAME=.*@HOSTNAME=$NOM_HOST_MACHINE_GLASSFISH1@" /etc/sysconfig/network

# TODO Vagrant garder que le "else" pour la PROD
if grep -c '^vagrant:' /etc/passwd; then
#change le hostname
  sudo sed -i 's/centos66/$NOM_HOST_MACHINE_GLASSFISH1/g' /etc/hosts
else
  sudo sed -i 's/centos66/$NOM_HOST_MACHINE_GLASSFISH1/g' /etc/hosts
fi
# TODO FIN Vagrant 
...
sudo hostname $NOM_HOST_MACHINE_GLASSFISH1

# installation de la partie glassfish
sh $BASE_SCRIPT_PROVISION/commun/./99-provision-glassfish.sh
sh $BASE_SCRIPT_PROVISION/glassfish1/05-glassfish1_glassfish.sh
