...
echo "Changement hostname definition du nom $NOM_HOST_MACHINE_GLASSFISH3:"
sudo sysctl kernel.hostname=$NOM_HOST_MACHINE_GLASSFISH3
...
sudo echo "127.0.1.1 $NOM_HOST_MACHINE_GLASSFISH3" >> /etc/hosts
sudo echo "127.0.0.1 $NOM_HOST_MACHINE_GLASSFISH3" >> /etc/hosts
...
sudo sed -i -e"s@HOSTNAME=.*@HOSTNAME=$NOM_HOST_MACHINE_GLASSFISH3@" /etc/sysconfig/network


# TODO Vagrant garder que le "else" pour la PROD
if grep -c '^vagrant:' /etc/passwd; then
#change le hostname
  sudo sed -i 's/centos66/$NOM_HOST_MACHINE_GLASSFISH3/g' /etc/hosts
else
  sudo sed -i 's/centos66/$NOM_HOST_MACHINE_GLASSFISH3/g' /etc/hosts
fi
# TODO FIN Vagrant 
...
sudo hostname $NOM_HOST_MACHINE_GLASSFISH3
